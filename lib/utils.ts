...
const WIDTH_SIZE = [25, 800];
const HEIGHT_SIZE = [25, 400];
const FONT_SIZE = [10, 32];
...
export function getRandomAttributes(content: string): DisplayerAttributes {
    ...
    const textTop = Math.floor(Math.random() * (containerHeight * 0.25 + 1));
    const textLeft = Math.floor(Math.random() * (containerWidth * 0.25 + 1));
